 ...
 * @describe：
 */
public class Type01DaoImpl implements AccidentAnalyseDao {
    @Override
    public List<AccidentAnalyse> getAll(AccidentCondition ac) {
        ...
        String sql1 = "select c.type01 as 'name',count(*) as 'value' from accident a,company c";
        String sql2 = " where a.company=c.name";
        String sql3 = " and";
        String sql4 = " type = ?";
        String sql5 = " time > ?";
        String sql6 = " time < ?";
        String sql7 = " group by c.type01";

        try {
            if (bType && bBegin && bEnd) {
                return queryRunner.query(sql1+sql2+sql7,new BeanListHandler<>(AccidentAnalyse.class));
            }
            if (!bType && bBegin && bEnd) {
